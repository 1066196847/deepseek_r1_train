...
def equation_reward_func(completions, target, nums, **kwargs):
    ...
    for completion, gt, numbers in zip(completions, target, nums):
        try:
            ...
            match = re.search(r"<answer>(.*?)<\/answer>", completion) # <answer>2 + 2</answer>
            if match is None:
                rewards.append(0.0)  # 如果没有匹配到 <answer> 标签，奖励为 0
                continue
            equation = match.group(1).strip()  # 提取 <answer> 标签中的内容   2 + 2
            # 提取方程中的所有数字
            used_numbers = [int(n) for n in re.findall(r"\d+", equation)] # ["2", "2"]
...
def grpo_function(
    ...
    # 加载分词器
    # dataset_args.tokenizer_name_or_path取值是空，真正用的是model_args.model_name_or_path
    ...
    model_kwargs = dict(
        revision=model_args.model_revision, # main   指定加载模型的Git分支或版本号
        trust_remote_code=model_args.trust_remote_code,
        attn_implementation=model_args.attn_implementation, # flash_attention_2
        torch_dtype=torch_dtype,
        # 启用梯度检查点技术，通过牺牲部分计算时间换取显存节省（约减少30%显存）
        use_cache=False if training_args.gradient_checkpointing else True,
    )
    # training_args的类型是：GRPOConfig
